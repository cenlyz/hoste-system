 ...
 //   			getChild();
    		...
    		createHead: function(fixColumns) {
    			...
    			$.each(fixColumns, function(index, fixColumn) {
    				var width = fixColumn.width;
    				if(width){
    					thead += ('<th width="'+width+'">' + fixColumn.name + '</th>');
    				}else{
    		...
    		createRow: function(rowData, fixColumns, alias) {
    			var _this = this;
    			var tr = '<tr  data-tt-id="' + rowData[alias.id] + '" data-tt-parent-id="' + (rowData.pid == undefined ? rowData[alias.pid] : rowData.pid) + '">';
    			tr += _this.createCols(rowData, fixColumns,alias);
    			tr += '</tr>';
    		 ...
    		 */
    		createCols: function(rowData, fixColumns,alias) {
    			var _this = this;
    			var cols = '';
    			$.each(fixColumns, function(index, fixColumn) {
    				if(fixColumn.title.substring(0,1)=='#'){
    					var view = $(fixColumn.title).html();
    					layui.laytpl(view).render(rowData,function(html){
    						cols += ('<td id="'+rowData[alias.id]+'">' + html + '</td>');
    					});
