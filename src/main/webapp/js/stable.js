...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'stable', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
    ...
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
      ...
      ,'{{# layui.each(d.data.cols, function(i1, item1){ }}'
        ...
        ,'{{# layui.each(item1, function(i2, item2){ }}'
          ...
          ,'{{# } else if(item2.radio){ }}'
            ,'<th data-field="{{ item2.field||i2 }}" data-type="radio" {{#if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} unresize="true"><div class="layui-table-cell laytable-cell-radio"><input type="radio" name="layTableRadio" lay-skin="primary" disabled lay-filter="layTableAllChooseR" {{# if(item2[d.data.radioName]){ }}checked{{# }; }}></div></th>'            
          ,'{{# } else if(item2.space){ }}'
  ...
  Class.prototype.events = function(){
    ...
    that.elem.on('click', 'input[name="layTableRadio"]+', function(){
      if ($(this).prev().attr("lay-filter")=="layTableAllChooseR") return;
      var radio = $(this).prev()
