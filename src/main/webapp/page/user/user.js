...
layui.config({
    ...
    if(window.sessionStorage.getItem('admin')){
        var userInfo = JSON.parse(window.sessionStorage.getItem('admin'));
        var citys;
        ...
        $(".userBirthday").val(userInfo.year); //出生年月
    ...
    form.on("submit(changeUser)",function(data){
        ...
        admin = {
            ...
            'sex' : data.field.sex,
            'phone' : $(".userPhone").val(),
            'year' : $(".userBirthday").val(),
            'email' : $(".userEmail").val(),
            'appraiser' : $(".myself").val().replace(/^\s+|\s+$/g,"")
        };
        var myselect=document.getElementById("province"); 
        var index=myselect.selectedIndex;
        var province=myselect.options[index].text;
        var mycity=document.getElementById("city"); 
        var indexcity=mycity.selectedIndex;
        var city=mycity.options[indexcity].text;
        var myarea=document.getElementById("area"); 
        var indexarea=myarea.selectedIndex;
        var area=myarea.options[indexarea].text;
        var site=province+city+area;
        admin.site=site;
        console.log(admin);
        ...
        window.sessionStorage.setItem("userInfo",JSON.stringify(admin));
        $.ajax({  
            url:"../../userAddOrEdit.do",  
            data:admin,  
            cache: false,  
            //async:false,  
            dataType:'json',  
    ...
    form.on("submit(changePwd)",function(data){
    	...
    	$.getJSON("../../changePwd.do",{"password":pwd,"username":username},function(data){
    		if(data.message=="ok"){
    		var index = layer.msg('提交中，请稍候',{icon: 16,time:false,shade:0.8});
                layer.close(index);
                layer.msg("密码修改成功！");
...
[i].provinceName + '_' + areaData[i].mallCityList.length + '_' + i + '">' + areaData[i].provinceName + '</option>';
    }
    //初始化省数据
